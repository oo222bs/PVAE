...
def read_input_folder(images_path, channel):
    all_file_list = []
    dir_list = []
    ...
    for file_list in all_file_list:
        ...
        for i, image in enumerate(file_list):
            ...
            resized_image_arr = np.asarray(original_image, np.float) / 255.0
            resized_images.append(np.expand_dims(resized_image_arr[:,:,channel], axis=2))
...
def deconv_decoder(dense, scope="deconv_decoder"):
    with tf.variable_scope(scope, reuse=False):
        ...
        output = tf.layers.conv2d_transpose(third_deconv, 1, 4, 2, padding='same', activation=tf.nn.sigmoid, name="dec_conv4_output")
    return output
...
def extract_visual_features():
    ...
    channel = 0
    resized_input, filenames = read_input_folder(im_data_dir, channel)
    batchsize = 1
    ...
    for i in range(resized_input.shape[0]):
        ...
        name = "../train/visual_feature_extraction_blue/"+ date +"/"+ filename + ".txt"
...
def reconstruct():
    ...
    for i in range(resized_input.shape[1]):
        ...
        if not os.path.exists(os.path.dirname(save_name)):
            os.makedirs(os.path.dirname(save_name))
        reconstructed_im = Image.fromarray(np.uint8(result[0,:,:,0]*255), mode='P')
