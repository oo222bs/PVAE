...
def main():
    ...
    L_dec_init_state = fc(VB_shared, L_num_units*L_num_layers*2,        # Initial decoder state for descriptions (h0_dec)
    ...
    for i in range(B_shape[1]):
        ...
        save_latent(np.transpose(result[0], (1, 0, 2)), L_filenames[i], "recognition")      # save the predicted descriptions
        r = result[0][:,0,:].argmax(axis=1)
        t = L_fw[1:,i,:].argmax(axis=1)
        print((r == t).all())        # Check if predicted descriptions match the original ones
